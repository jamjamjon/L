...
def tvt_split(	img_folder, ann_folder=None, 
	...
	# test dir
	for test_id in tqdm(test_samples_id):
...
# train_val split
def train_val_split_darknet(img_folder, 
								# ann_folder=None, 
								rate=0.8, 
								seed=777, 
								cp_mv = "cp", 
	...
	# make save dirs for train_val_images
	Path(train_dir).resolve().mkdir(parents=True, exist_ok=True)
	Path(val_dir).resolve().mkdir(parents=True, exist_ok=True)	

	# 
	all_images = [x for x in list(Path(images_dir).iterdir()) if not str(x).endswith(".txt")]	# all images
	num_images = len(all_images)					# num of all images
	num_train = int(num_images * rate)		# num of train images
	num_val = num_images - num_train	# num of val images

	# info
	rich.print(f"[bold magenta]==>All element: {num_images} | Train number: {num_train} | Val number: {num_val}")

	
	# random select
	random.seed(seed)
	train_samples_id = random.sample(range(num_images), num_train)
	val_samples_id = list(set(range(num_images)) - set(train_samples_id))
	...
	for train_id in tqdm(train_samples_id):
		...
		if cp_mv == "cp":
			...
			ann_path = images_dir / Path(str(image.stem) + ".txt")
			shutil.copy(ann_path, train_dir)
		elif cp_mv == "mv":
			shutil.move(str(image), train_dir)
			ann_path = images_dir / Path(str(image.stem) + ".txt")
			shutil.move(str(ann_path), train_dir)
	...
	for val_id in tqdm(val_samples_id):
		...
		if cp_mv == "cp":
			...
			ann_path = images_dir / Path(str(image.stem) + ".txt")	
			shutil.copy(ann_path, val_dir)
		elif cp_mv == "mv":
			shutil.move(str(image), val_dir)
			ann_path = images_dir / Path(str(image.stem) + ".txt")
			shutil.move(str(ann_path), val_dir)
