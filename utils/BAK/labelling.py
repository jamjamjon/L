...
from general import (get_corresponding_label_path, Colors, load_img_label_list, set_logging, 
                    video_record, IMG_FORMAT, is_point_in_rect, LOGGER, img_label_dir_cleanup
                    )
...
def edit_bbox(bbox_with_id, action):
    ...
    for path in bboxes_to_edit_dict:
        ...
        yolo_line = yolo_format(cls_idx, (xmin, ymin), (xmax, ymax), img_width, img_height) # TODO: height and width ought to be stored
        ...
        with open(ann_path, 'w') as new_file:
            for idx, line in enumerate(lines):
                if idx != ind:              # 如果未修改的行，直接写入
                    ...
                    new_yolo_line = yolo_format(new_class_index, (xmin, ymin), (xmax, ymax), img_width, img_height)
...
#           mv_dir, wrong_img_dir,  可以放到main里面 
#          mark_mode,       貌似不可以
#          classes_txt,    可以放到main里面 
            # hide_label,    可以放到main里面 
            # hide_num_bboxes,  可以放到main里面 
            # blink-bboxes, 可以放到main里面 
            # cls   : SINGLE_CLS  可以放到main里面
#          ):
...
def inspect(
    ...
    # num_bboxes
    HIDE_NUM_BBOXES = hide_num_bboxes

    # only show one specific class
    SINGLE_CLS = cls

    # bboxes blink
    BLINK_BBOXES = blink_bboxes
    BLINK_OR_NOT = False
    ...
    # classes.txt
    CLASSES_TXT = classes_txt          # classes txt
...
#     # wrong dir & move dir
#     WRONG_IMG_DIR = opt.wrong_img_dir
#     MV_DIR = opt.mv_dir
